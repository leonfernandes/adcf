...
#' @export
acv_metric <- function(data, ...) {
    UseMethod("acv_metric")
}

#' @rdname acv_metric
#' @export
acv_metric.data.frame <- function(
    data,
    truth,
    ...,
...
acv_metric_impl <- function(truth, estimate, lag, case_weights = NULL) {
    z <- estimate - truth
    tibble::tibble(
        lag = 0:max(lag),
        acv = stats::acf(
            z,
            lag.max = max(lag), type = "covariance", plot = FALSE,
            demean = FALSE
        )$acf |>
            as.numeric()
    ) |>
